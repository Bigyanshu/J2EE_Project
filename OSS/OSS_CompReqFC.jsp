...
<%! 
    Connection conn;
    Statement stmtData, stmtReqNo,stmtCName,stmtPCode;
    ResultSet rsData, rsRqeNo,rsCName,rsPCode;
    String ReqNo1,CName2,PCode2,Date1[],CName1[],CRegdNo1[], Locality1[], City1[], State1[], GST1[],SlNo1[],PCode1[], PName1[],Category1[], Price1[],OQty1[], Amount1[];
    int i;
...
<%
    ...
    try 
	{
        ...
        while (rsData.next()) 
		{
            Date1[i] = rsData.getString("Date1");
            CName1[i] = rsData.getString("CName1");
            CRegdNo1[i] = rsData.getString("CRegdNo1");
            Locality1[i] = rsData.getString("Locality1");
            City1[i] = rsData.getString("City1");
            State1[i] = rsData.getString("State1");
            GST1[i] = rsData.getString("GST1");
            SlNo1[i] = rsData.getString("SlNo1");
            PCode1[i] = rsData.getString("PCode1");
			PName1[i] = rsData.getString("PName1");
			Category1[i] = rsData.getString("Category1");
            Price1[i] = rsData.getString("Price1");
            OQty1[i] = rsData.getString("OQty1");
            Amount1[i] = rsData.getString("Amount1");
...
<html>
	<head>
		<title>OSS Requisition Find Client</title>
		...
		<script language="javascript">
			...
			function goBack()
			{
				window.location="OSS_CompReqC.jsp";
			}
		</script>
	</head>
	<body>
		<center>
			<form name="f1" action="OSS_CompReqFS.jsp" method="post">
				...
				<table>
					...
					<tr>
						...
						<td colspan="2">: 
							<select name="cmbReqNo" onchange="fillCompReq()">
								...
								<%
									...
									try 
									{
										...
										rsRqeNo = stmtReqNo.executeQuery("SELECT * FROM oss_tblcompreq");
										while (rsRqeNo.next()) 
										{
											ReqNo1 = rsRqeNo.getString("ReqNo1");
								%>
							...
							<select name="cmbCName" onchange="fillCompany">
								...
								<%
									...
									try 
									{
										...
										while (rsCName.next()) 
										{
											CName2 = rsCName.getString("CName");
								%>
											<option value="<%= CName2 %>"><%= CName2 %></option>
								<%
										}
									} 
									catch (SQLException se)
					...
					<tr>
						...
						<td></td>
						<td>GST No</td>
						<td colspan="2">: <input type="text" name="txtGST" size="30" placeholder="Enter GST1 No"></td>
					</tr>
					<tr>
						...
						<td>
							...
							<% 
								try 
								{
									...
									while (rsPCode.next()) 
									{
										PCode2 = rsPCode.getString("PCode");
							%>
										<option value="<%=PCode2%>"><%=PCode2%></option>
							<% 
									}
								} 
								catch (SQLException se) 
